...
public class EndMiningTeleport extends Block {
    ...
    public InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hitResult) {
        ...
        ResourceKey<Level> miningKey = ResourceKey.create(Registries.DIMENSION, ResourceLocation.fromNamespaceAndPath(EveryMiningDimension.MOD_ID, "end_mining_dimension"));
    ...
    private InteractionResult teleportPlayer(Player player, ServerLevel currentWorld, ResourceKey<Level> targetKey, BlockPos fromPos) {
        ...
        BlockPos finalPos;
        if (targetKey.location().equals(ResourceLocation.fromNamespaceAndPath(EveryMiningDimension.MOD_ID, "end_mining_dimension"))) {
            finalPos = new BlockPos(fromPos.getX(), 64, fromPos.getZ());
            targetWorld.setBlock(finalPos, this.defaultBlockState(), 3);
            player.getPersistentData().putLong("overworldPortalPos", fromPos.asLong());
        } else {
            if (player.getPersistentData().contains("overworldPortalPos")) {
                finalPos = BlockPos.of(player.getPersistentData().getLong("overworldPortalPos"));
            } else {
                finalPos = player.blockPosition();
            }
        }
